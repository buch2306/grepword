...
def test_inverted_index_returns_correct_file_and_line(inverted_index):
    ...
    print(result)
    assert len(result) == 2, f"Expected 'hello' to appear in two files, got {len(result)}"
    
    # Checking if the line numbers match expectations
    assert ['sample1.txt', [1, 3]] in result, "'hello' should appear in sample1.txt, lines 1 and 3"
    assert ['sample2.txt', [1]] in result, "'hello' should appear in sample2.txt, line 1"
...
def test_inverted_index_word_count(inverted_index):
    ...
    result = inverted_index.inverted_index.get(word)
    print(result)
    assert len(result) == 2, f"Expected 'test' to appear in two files, got {len(result)}"
...
def test_inverted_index_empty_file(inverted_index):
    ...
    assert not empty_file_inverted_index.inverted_index, "Inverted index should be empty for an empty file"
